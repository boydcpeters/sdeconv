...
def hv_loss(img, weighting):
    ...
    """
    dxx2 = torch.square(
        -img[:, :, 2:, 1:-1] + 2 * img[:, :, 1:-1, 1:-1] - img[:, :, :-2, 1:-1]
    )
    dyy2 = torch.square(
        -img[:, :, 1:-1, 2:] + 2 * img[:, :, 1:-1, 1:-1] - img[:, :, 1:-1, :-2]
    )
    dxy2 = torch.square(
        img[:, :, 2:, 2:]
        - img[:, :, 2:, 1:-1]
    ...
    h_v = torch.sqrt(
        weighting * weighting * (dxx2 + dyy2 + 2 * dxy2)
        + (1 - weighting)
        * (1 - weighting)
        * torch.square(img[:, :, 1:-1, 1:-1])
    )
...
def hv_loss_3d(img, delta, weighting):
    ...
    d33 = (
        delta
        * delta
        * (-img[:, :, 2:, 1:-1, 1:-1] + 2 * img_ - img[:, :, :-2, 1:-1, 1:-1])
    )
    d12_d21 = (
        img[:, :, 1:-1, 2:, 2:]
        - img[:, :, 1:-1, 1:-1, 2:]
    ...
    h_v = (
        torch.square(weighting * d11)
        + torch.square(weighting * d22)
        + torch.square(weighting * d33)
        + 2 * torch.square(weighting * d12_d21)
        + 2 * torch.square(weighting * d13_d31)
        + 2 * torch.square(weighting * d23_d32)
        + torch.square((1 - weighting) * img_)
    )
...
class DataTermDeconv3D(torch.autograd.Function):
    ...
    def forward(
        ...
        conv_deblured_image = torch.real(
            torch.fft.ifftn(fft_deblurred_image * fft_psf)
        )
        mse = torch.nn.MSELoss()
    ...
    def backward(ctx, grad_output):
        ...
        real_tmp = (
            fft_psf.real * fft_deblurred_image.real
            - fft_psf.imag * fft_deblurred_image.imag
            - fft_blurry_image.real
        )
        imag_tmp = (
            fft_psf.real * fft_deblurred_image.imag
            + fft_psf.imag * fft_deblurred_image.real
            - fft_blurry_image.imag
        )

        residue_image_real = (
            adjoint_otf.real * real_tmp - adjoint_otf.imag * imag_tmp
        )
        residue_image_imag = (
            adjoint_otf.real * imag_tmp + adjoint_otf.imag * real_tmp
        )

        grad_ = torch.real(
            torch.fft.ifftn(
                torch.complex(residue_image_real, residue_image_imag)
            )
        ) / torch.numel(deblurred_image)
...
class Spitfire(SDeconvFilter):
    ...
    def __call__(self, image, mini: int = None, maxi: int = None):
        if image.ndim == 2:
            return self.run_2d(image, mini, maxi)
        if image.ndim == 3:
            return self.run_3d(image, mini, maxi)
        raise Exception("Spitfire can process only 2D or 3D tensors")

    def run_2d(self, image, mini: int = None, maxi: int = None):
        ...
        # If the minimum and maximum intensity are not provided by the user,
        # determine them from the provided image.
        if mini is None:
        ...
        image_pad, psf_pad, padding = pad_2d(
            image, self.psf / torch.sum(self.psf), self.pad
        )
        psf_pad = self.psf / torch.sum(self.psf)

        image_pad = image_pad.view(
            1, 1, image_pad.shape[0], image_pad.shape[1]
        )
        ...
        for i in range(self.max_iter_):
            ...
            self.niter_ += 1
            optimizer.zero_grad()
            loss = self.reg * dataterm_deconv(
                image_pad, deconv_image, psf_pad
        ...
        deconv_image = deconv_image.view(
            deconv_image.shape[2], deconv_image.shape[3]
        )
        if (
            image_pad.shape[2] != image.shape[0]
            and image_pad.shape[3] != image.shape[1]
        ):
            return deconv_image[
    ...
    def adjoint_otf(psf):
        ...
        adjoint_psf = torch.roll(
            adjoint_psf, -int(psf.shape[0] - 1) % 2, dims=0
        )
        adjoint_psf = torch.roll(
            adjoint_psf, -int(psf.shape[1] - 1) % 2, dims=1
        )
        adjoint_psf = torch.roll(
            adjoint_psf, -int(psf.shape[2] - 1) % 2, dims=2
        )
    ...
    def run_3d(self, image, mini: int = None, maxi: int = None):
        ...
        image = (image - mini) / (maxi - mini)
        image_pad, psf_pad, padding = pad_3d(
            image, self.psf / torch.sum(self.psf), self.pad
        )

        deconv_image = image_pad.detach().clone()
        image_pad = image_pad.view(
            1, 1, image_pad.shape[0], image_pad.shape[1], image_pad.shape[2]
        )
        deconv_image = deconv_image.view(
            1,
            1,
            deconv_image.shape[0],
            deconv_image.shape[1],
            deconv_image.shape[2],
        )
        deconv_image.requires_grad = True
        optimizer = torch.optim.Adam([deconv_image], lr=self.gradient_step_)
        scheduler = torch.optim.lr_scheduler.StepLR(
        ...
        for i in range(self.max_iter_):
            ...
            self.niter_ += 1
            optimizer.zero_grad()
            loss = self.reg * dataterm_(
                deconv_image, image_pad, fft_image, fft_psf, adjoint_otf
            ) + (1 - self.reg) * hv_loss_3d(
        ...
        self.progress(100)
        deconv_image = deconv_image.view(
            image_pad.shape[2], image_pad.shape[3], image_pad.shape[4]
        )
        deconv_image = (maxi - mini) * deconv_image + mini
        if (
            image_pad.shape[2] != image.shape[0]
            and image_pad.shape[3] != image.shape[1]
            and image_pad.shape[4] != image.shape[2]
        ):
...
def spitfire(
    ...
    if isinstance(image, np.ndarray):
        psf_ = torch.tensor(psf).to(device)
    else:
        psf_ = psf
    ...
    if isinstance(image, np.ndarray):
        return filter_(torch.tensor(image).to(device))
...
metadata = {
    ...
    "fnc": spitfire,
    "inputs": {
        "image": {"type": "Image", "label": "Image", "help": "Input image"},
        "psf": {
            "type": "Image",
            "label": "PSF",
    ...
    "outputs": {
        "image": {"type": "Image", "label": "Spitfire"},
    },
}
